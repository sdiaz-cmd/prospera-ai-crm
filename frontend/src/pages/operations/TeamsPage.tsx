...
function CreateCuadrillaModal({ onClose, users }: { onClose: () => void; users: UserOption[] }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="bg-[#0f1623] border border-white/[0.1] rounded-2xl w-full max-w-md shadow-2xl">
        ...
        <form onSubmit={e => { e.preventDefault(); if (name.trim()) mut.mutate({ name: name.trim(), description, chiefId }); }} className="p-5 space-y-4">
          ...
          <div className="flex justify-end gap-3">
            <button type="button" onClick={onClose} className="px-3 py-1.5 text-sm text-gray-500 hover:text-gray-300">Cancelar</button>
    ...
    <div className="bg-white/[0.02] border border-white/[0.07] rounded-2xl overflow-hidden">
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <p className="text-sm font-semibold text-gray-200">{cuadrilla.name}</p>
          <div className="flex items-center gap-3 mt-0.5">
            {cuadrilla.chiefName && <span className="text-xs text-gray-600">Jefe: {cuadrilla.chiefName}</span>}
      ...
      {expanded && (
        <div className="px-5 pb-4 border-t border-white/[0.06] pt-4 space-y-3">
          ...
          <div className="flex items-center justify-between">
            <p className="text-xs font-semibold text-gray-600 uppercase tracking-wider">Miembros</p>
            <button onClick={() => setAddingMember(a => !a)} className="flex items-center gap-1 text-xs text-blue-400 hover:text-blue-300 transition-colors">
          ...
          {members.length === 0 ? (
            ...
            <div className="space-y-1">
              {members.map(m => (
                <div key={m.userId} className="flex items-center gap-3 px-3 py-2 rounded-lg bg-white/[0.02] group">
                  ...
                  <span className="flex-1 text-sm text-gray-300">{m.firstName} {m.lastName}</span>
    ...
    <div className="p-6 space-y-5">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-gray-600 mt-0.5">{cuadrillas.length} cuadrilla{cuadrillas.length !== 1 ? 's' : ''}</p>
        </div>
        <button
      ...
      {isLoading ? (
        <div className="py-12 text-center text-gray-600 text-sm">Cargando cuadrillas...</div>
      ) : cuadrillas.length === 0 ? (
